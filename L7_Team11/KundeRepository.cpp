...
*/
vector<Auto> KundeRepository::filterAlter(const int& alter) const {
    int idx = 0;
    vector<Auto> output = {};
    while (idx < this->autos.size()) {
...
*/
vector<Auto> KundeRepository::filterKilometer(const int& kilometer) const {
    int idx = 0;
    vector<Auto> output = {};
    while (idx < this->autos.size()) {
...
*/
vector<Auto> KundeRepository::zeigeFavoriten() const {
...
*/
vector<Auto> KundeRepository::zeigeNachPreisSortiert() {
    sort(this->autos.begin(), this->autos.end(), [](Auto elem1, Auto elem2) { return (elem1.get_preis() < elem2.get_preis()); });
    return this->autos;
}


void KundeRepository::leseAusDateien(string dFile, string fFile) {
    ifstream datei;
...
void KundeRepository::schreibeInDateien(string dFile, string fFile) {
    // Offnen der Datei mit Autos
