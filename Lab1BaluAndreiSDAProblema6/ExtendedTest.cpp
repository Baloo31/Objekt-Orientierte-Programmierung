...
void testRandomAccessIterator() {
	cout << "Test Random Access Iterator" << endl;
	Bag b;
	// Nu permite construirea iteratorului cu pozitii invalide
	try {
		RandomAccessIterator iter = b.iteratorRandom(-3);
		assert(false);
	}
	catch (exception&) {
		assert(true);
	...
	Bag b1;
	RandomAccessIterator iter = b1.iteratorRandom(0);
	assert(iter.valid());
	b.add(1);
	b.add(2);
	b.add(3);
